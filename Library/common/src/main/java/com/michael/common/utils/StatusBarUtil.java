...
public class StatusBarUtil {
     ...
     * @param color    色值
     */
    public static void addTopView(Activity activity, @ColorRes int color) {
        ViewGroup contentView = (ViewGroup) activity.getWindow().getDecorView();
        addTopView(contentView, activity, color);
    }

    public static <T extends ViewGroup> void addTopView(T contentView, Activity activity, @ColorRes int color) {
        // 移除上一次视图
        if (contentView.getChildCount() > 1) {
            contentView.removeViewAt(0);
        }
        contentView.addView(createStatusBarView(activity, color), 0);
    }
